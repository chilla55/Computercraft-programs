...
To select an exact release, use `install-transformer.lua transformer distributed-1.1.10` with the current installer. The optional second argument reads the manifest from that immutable tag and rejects a different version. Without it, the installer checks the latest manifest using a timestamped URL to avoid stale caches. Choose an unused folder; existing installations are never overwritten.
...
4. Calculates absolute startup destinations for all three banks from one input/position snapshot, accounting for the exit ratio and whole-degree movement resolution. It starts all required bank drives together, using full-travel commands (for example, 200 degrees to 1 degree is one 199-degree move). Output breakers remain open. It then waits for every shaft to stop, verifies exact bank alignment and destination accuracy, and checks measured output voltage. Up to two recalculated correction plans account for model/measurement differences; otherwise it trips with the measured/target voltage or an unreachable-target reason. No further plan starts before the entire previous one is checked. The separate protection computer continues sampling every variac temperature during planning and motion; neither a full-travel command nor waiting for shafts to stop suspends the thermal trip curve.
